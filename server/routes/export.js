...
router.get('/markdown', wrap((req, res) => {
  ...
  const tasks = queryAll(`
    ...
    FROM tasks t LEFT JOIN user_stories s ON t.story_id = s.id
    WHERE s.project_id = ?
  ...
  const meetings = queryAll('SELECT * FROM meetings WHERE project_id = ? ORDER BY meeting_date DESC', [projectId]);
