...
import { Target, CheckCircle2, ListTodo, Flame, Calendar, AlertTriangle, TrendingUp, Clock, Zap, ThumbsUp, Plus, Trash2 } from 'lucide-react';
        ...
        <div className="card col-span-3 p-5" style={{ animation: 'slideUp .5s var(--spring-bounce) .15s both' }}>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-4 text-xs">
              ...
              <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 rounded" style={{ background: '#ea580c' }} />实际</span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={burndownData}>
              ...
              <Tooltip contentStyle={{ borderRadius: 16, border: '1px solid rgba(255,255,255,0.8)', background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(30px) saturate(200%)', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', fontSize: 12 }} cursor={{ stroke: '#ea580c', strokeWidth: 1, strokeDasharray: '4 4' }} />
              <Area type="monotone" dataKey="ideal" stroke="#cbd5e1" strokeDasharray="6 4" strokeWidth={1.5} dot={false} fill="none" />
              <Area type="monotone" dataKey="actual" stroke="#ea580c" strokeWidth={2.5} fill="url(#burndown)" dot={{ r: 3, fill: '#ea580c', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 5, fill: '#ea580c', stroke: '#fff', strokeWidth: 2 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card col-span-2 p-5" style={{ animation: 'slideUp .5s var(--spring-bounce) .2s both' }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={sprintData} barGap={4}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ borderRadius: 16, border: '1px solid rgba(255,255,255,0.8)', background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(30px) saturate(200%)', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', fontSize: 12 }} cursor={{ fill: 'rgba(234,88,12,0.05)' }} />
              <Bar dataKey="计划" fill="rgba(226,232,240,0.5)" radius={[6, 6, 0, 0]} />
              <Bar dataKey="完成" fill="#ea580c" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
                ...
                {celebrating === m.id && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <div className="animate-thumbs-up">
                      <div className="text-5xl animate-thumbs-pulse" style={{ filter: 'drop-shadow(0 4px 12px rgba(234,88,12,0.3))' }}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-3" style={{ borderTop: '1px solid rgba(241,245,249,0.5)' }}>
        ...
        <button onClick={() => { if (validate()) onCreate(form); }} className="btn btn-primary">创建</button>
      </div>
    </div>
  );
}
...
function StatCard({ icon, iconClass, label, value, sub, color, trend, delay = 0 }) {
  const colors = {
    indigo: { gradient: 'linear-gradient(135deg, rgba(234,88,12,0.08), rgba(234,88,12,0.15))', text: 'text-orange-600', glow: 'rgba(234,88,12,0.15)' },
