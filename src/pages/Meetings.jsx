...
import { Plus, Calendar, User, MessageSquare, CheckSquare, Trash2, Edit2, Clock, ArrowRight } from 'lucide-react';
    ...
    <div className="space-y-5 max-w-[1400px]">
      ...
      <div className="relative">
        <div className="absolute left-[18px] top-0 bottom-0 w-px"
          style={{ background: 'linear-gradient(180deg, rgba(234,88,12,0.3), rgba(241,245,249,0.3))' }} />
            ...
            return (
              <div key={m.id} className="relative pl-12 group"
                ...
                <div className="absolute left-[11px] top-5 w-[15px] h-[15px] rounded-full border-2 flex items-center justify-center"
                  style={{
                    borderColor: isPast ? 'rgba(203,213,225,0.5)' : '#ea580c',
                    background: isPast ? 'rgba(255,255,255,0.8)' : '#fff',
                    boxShadow: !isPast ? '0 0 0 4px rgba(234,88,12,0.1)' : 'none',
                  }}>
                  {!isPast && <div className="w-2 h-2 rounded-full bg-orange-500" />}
                </div>

                <div className={`card p-5 transition-all duration-500 ${isPast ? 'opacity-70 hover:opacity-100' : 'ring-1 ring-orange-100/40'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                        style={{ background: isPast ? 'rgba(148,163,184,0.1)' : 'rgba(234,88,12,0.08)' }}>
                  ...
                  {m.action_items && (
                    <div className="rounded-xl p-3.5 mb-3" style={{ background: 'rgba(234,88,12,0.04)' }}>
  ...
  return (
    <div className="space-y-4 max-h-[60vh] overflow-y-auto scrollbar-hide pr-1">
      ...
      <div className="flex justify-end gap-2 pt-3" style={{ borderTop: '1px solid rgba(241,245,249,0.5)' }}>
        ...
        <button onClick={() => { if (validate()) onSave(form); }} className="btn btn-primary">保存</button>
      </div>
    </div>
  );
}
